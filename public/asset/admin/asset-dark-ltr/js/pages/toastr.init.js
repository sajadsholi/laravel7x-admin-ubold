...
function(i) {
    ...
    i("#toastr-one").on("click", function(t) { i.NotificationApp.send("Heads up!", "This alert needs your attention, but it is not super important.", "top-right", "#3b98b5", "info") }),
        ...
        i("#toastr-ten").on("click", function(t) { i.NotificationApp.send("Plain transition", "Set the `showHideTransition` property to fade|plain|slide to achieve different transitions.", "top-right", "#3b98b5", "success", 3e3, 1, "plain") })
    ...
    $(function() {
        ...
        if ($('#success-msg').length > 0) {
            i.NotificationApp.send("", $('#success-msg').html(), "top-right", "#5ba035", "success", 7e3, 1, "plain");
        }
        // info toaster
        if ($('#info-msg').length > 0) {
            i.NotificationApp.send("", $('#info-msg').html(), "top-right", "#1ea69a", "info", 12e3, 1, "plain");
        }
        // failed toaster
        if ($('#failed-msg').length > 0) {
            i.NotificationApp.send("", $('#failed-msg').html(), "top-right", "#bf441d", "error", 12e3, 1, "plain");
        }
    });
